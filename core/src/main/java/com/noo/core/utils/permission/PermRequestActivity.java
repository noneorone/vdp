...
public class PermRequestActivity extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (perms != null && perms.length > 0) {
            PermChecker.RequestPermCallback permCallback = new PermChecker.RequestPermCallback() {
                @Override
            ...
            boolean hasPermissionAppOps = PermChecker.hasPermissionAppOps(this, perms);
            if (hasPermissionAppOps) {
                permCallback.onRequestSuccess();
            } else {
                permChecker = new PermChecker(this);
                permChecker.requestPermissions(PERM_REQUEST_CODE, perms, permCallback);
            }
        } else {
            if (callback != null) {
                callback.onRequestFail();
